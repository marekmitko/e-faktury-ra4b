...
export const CreateInvoiceButton = (props: any) => {
    const { onClick } = props;
    ...
    // https://codesandbox.io/s/createinvoicebutton-ip1q02?file=/demo.tsx:1716-1739
        ...
        const handleOnPrepared = (onClick : any,  ) => (event: React.FormEvent<HTMLFormElement>) => {
            // event.preventDefault();
            ...
            try {
                ...
                setButtonState((current) => ({ ...current, status: 'failure' }));
            }
      };
    ...
    <>
        ...
        > */}
                    ...
                    // disabled={loading}
                        ...
                        // size="md"
                        sx={{ pl: 1,     "--Button-gap": "5px", lineHeight: 0.5, textTransform: 'uppercase'
                    }}
                    {...props} 
                    onClick={handleOnPrepared(onClick)}
                    >
                        {   (buttonState.status === 'loading') ?  translate('myroot.custom_ra.action.button.invoicePreparation'):
